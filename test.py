...
pkl_list = []
with open("/root/bjy/BjyFiles/LavalTrain/data_helper/laval_test_list.txt", 'r') as f:
	lines = f.readlines()
	for line in lines:
		line = line.strip()
...
def output():
    for idx in range(test_len):
        ...
        for w in range(5):
            index = idx * 5 + w
            save_path = save_dir + "/" + str(index) + "_input.png"
            cv2.imwrite(save_path, origin_img[w])
            save_path = save_dir + "/" + str(index) + "_gt.exr"
            cv2.imwrite(save_path, true_image[w])
            save_path = save_dir + "/" + str(index) + "_predict.exr"
